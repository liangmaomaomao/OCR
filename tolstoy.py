...
def correct(fraction_str):
    try:
        if '/' in fraction_str:
            numerator, denominator = fraction_str.split('/')
            set1 = [numerator, denominator]
            lists = [['1', '2'], ['3', '4']]
            lis_return = ['1/2', '3/4']
            for i in range(2):
                if find_common_and_return(set1, lists[i], lis_return[i]) is not None:
                    return find_common_and_return(set1, lists[i], lis_return[i])
        else:
            return 1
    except ValueError:
        return None
...
def find_common_and_return(list1, list2, return_value):
...
if __name__ == '__main__':
    ...
    jpeo = 'image1.jpeg'
    jpet = 'image2.jpeg'
    out = 'spreadsheet.xlsx'
    endpoint = 'https://xinyu.cognitiveservices.azure.com/'
    subscription_key = '2a3a3275fae84653b4a90fab610c6e81'
    ...
    for image in images:
        ...
        renew_date = datetime.strptime(
            re.search(r".*DATE(.*?)FOREMAN", image_data1_text, re.IGNORECASE).group(1).replace(" ", ""),
